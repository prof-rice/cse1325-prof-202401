...
public class Factor {
    ...
    public static void main(String[] args) {
        ...
        // Create a thread pool to factor all of the big integers
        ...
        for(Thread thread : threads) {
            try {
                ...
                System.err.println("Exception joining thread " + thread + ": " + e);
            }
        }
    ...
    private static int nextBigInt = 0;
    public synchronized static int getBigIntIndex() {
        if(nextBigInt < bigints.length) return nextBigInt++;
        return -1;
    }
    
    public static void worker(int threadNumber) {
        int index = 0;
        while((index = getBigIntIndex()) != -1) {
            //  solve(threadNumber, index, index+1); // OR
            System.out.println("Thread " + threadNumber + " solving " + index);
            try {
                PrimeFactors pf = new PrimeFactors(new BigInteger(bigints[index]));
                synchronized(solutions) {
                    solutions.add(pf);
                }
            } catch (Exception e) {
                System.err.println("Failed to solve " + bigints[index] + ": " + e);
            }
            System.out.println("Thread " + threadNumber + " finished " + index);
        }
        System.out.println("Thread " + threadNumber + " exited");
    }
