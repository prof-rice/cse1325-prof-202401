...
public class Factor {
    ...
    public static void main(String[] args) {
        ...
        Double delta = ((double) bigints.length) / (double) numThreads;
        ...
        for(int threadNum=0; threadNum<numThreads; ++threadNum) {
            ...
            int first = last;
            next += delta;
            if(threadNum < numThreads-1) last = Math.min(next.intValue(), bigints.length);
            else last = bigints.length;
            final int t = threadNum;
            final int f = first;
            final int l = last;
            threads.add(new Thread(()-> solve(t,f,l)));
        ...
        for(Thread t : threads) {
            try {
                ...
                System.err.println("Exception joining thread " + t + ": " + e);
            }
        }
    ...
    // Add solutions to the solutions ArrayList
